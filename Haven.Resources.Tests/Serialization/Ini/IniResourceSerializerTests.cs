...
namespace Haven.Resources.Serialization.Ini
{
	...
	{
		private const string BasePath = "Samples/Ini";
		...
		public void ImageDataTest()
		{
			using (var fs = File.OpenRead(Path.Combine(BasePath, "image.ini")))
			{
				...
				Assert.That(layers.Length, Is.EqualTo(1), "Wrong number of layers");
				Assert.That(layers[0], Is.InstanceOf(typeof(Resources.ImageLayer)), "Wrong layer type");

				var imageLayer = (Resources.ImageLayer)layers[0];
